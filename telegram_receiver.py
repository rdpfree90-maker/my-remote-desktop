...
# The GitHub runner account and the RDP account are not necessarily the same.
# Always target the RDP user's real Desktop so uploaded files are visible in RDP.
if RDP_USERNAME:
    RDP_PROFILE = Path(os.environ.get("RDP_PROFILE", rf"C:\Users\{RDP_USERNAME}"))
    DESKTOP = RDP_PROFILE / "Desktop"
else:
...
for folder in (TELEGRAM_DIR, ANDROID_PROJECTS, ANDROID_INCOMING, ANDROID_APKS):
...
def menu():
    ...
    kb.add(types.InlineKeyboardButton("📁 ملفات Telegram", callback_data="mode:telegram"))
    kb.add(types.InlineKeyboardButton("🤖 Android Studio", callback_data="mode:android"))
    return kb
...
def safe_extract_zip(zf: zipfile.ZipFile, destination: Path):
    ...
    for member in zf.infolist():
        ...
        if not raw or raw.endswith("/"):
            continue
        # Prevent absolute paths and ../ traversal (Zip Slip).
        p = Path(raw)
...
def extract_android_project(zip_path: Path, project_name: str) -> Path:
    ...
    try:
        ...
        # If the ZIP has a single top-level project folder, remove that wrapper
        # so Android Studio opens the real project directly.
        if root and (temp_dir / root).is_dir():
            source = temp_dir / root
        else:
            source = temp_dir

        if not looks_like_android_project(source):
            # Still keep the files; the user may have a non-standard project.
            pass
...
def start(message):
    ...
    bot.send_message(
        ...
        "اختار المكان اللي عاوز ترفع له:\n\n"
        "📁 <b>ملفات Telegram</b> → الملفات العادية والروابط\n"
        "🤖 <b>Android Studio</b> → ZIP مشروع Android / APK / ملفات التطوير\n\n"
        "في وضع Android Studio، لما تبعت ZIP مشروع هيتحفظ ويتفك تلقائيًا داخل Projects.",
        parse_mode="HTML",
        reply_markup=menu(),
    )
...
def change_mode(call):
    ...
    if selected == "android":
        text = (
            "🤖 <b>وضع Android Studio</b>\n\n"
            "📦 ابعت ZIP مشروع Android → هيتفك تلقائيًا.\n"
            "📱 ابعت APK → هيتحفظ في APKs.\n"
            "📄 أي ملف آخر → Incoming.\n\n"
            "📂 مكان المشاريع:\n<code>Desktop\\AndroidStudio\\Projects</code>"
        )
    else:
        text = (
            "📁 <b>وضع ملفات Telegram</b>\n\n"
            "أي ملف أو رابط هيتحفظ داخل:\n<code>Desktop\\Telegram</code>"
        )
